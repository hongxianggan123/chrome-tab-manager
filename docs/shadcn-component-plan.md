...
| 设置入口 | `Button`, 受控设置视图, `RadioGroup` | header 齿轮按钮进入可扩展设置视图，当前包含重复提示展示方式。 |

## 不使用或后置

- MVP 不使用 `Card` 包裹标签行。标签行需要高密度列表形态，卡片会增加视觉噪音。
- MVP 不使用 `Table`。标签清单不是表格数据，行内有动作、多行文本和悬浮选择入口。
- MVP 不使用 `Dialog`、`AlertDialog`。关闭和归档不做二次确认。
- MVP 和本阶段设置视图不使用 `Sheet`。Chrome Side Panel 本身已经是侧边栏容器，设置使用受控视图替换主列表区域。
...
- `SettingsPanel`：侧边栏内可扩展设置面板，按能力分区组织配置。
...
这些 token 保留给少量状态表达和历史视觉方案。正文、边框、背景、muted 文本仍优先使用 shadcn 默认语义 token。
